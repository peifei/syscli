<?php
class Application_Service_Fileimport_FxcmDataer extends 
Application_Service_Fileimport_DataAbstract{
    ...
    public function doimport($month){
        ...
        $selecter=$db2->select()->from('fxcm_merchant_activity_summary')->where("symbol!='' and volume_closed!='0.00'");
...
/*    public function doimport($month){
    ...
    public function insertData($db,$data,$month){
        $count=$db->fetchRow("select count(*) as counter from fxcmdata where month='".$month."'");
        //先判断当前操作的月份数据是否在表中已有记录，如果没有表示开始一个新月份则删除以前的数据
        if(0==$count['counter']){
            $db->query('delete from fxcmdata;');
            echo 'trunate executed';
        }
        //查询是否存在原有数据
        $res=$db->fetchRow("select * from fxcmdata where account='".$data['account']."' and symbol='".$data['symbol']."' and month='".$month."'");
        ...
        if($res){
            $resData=$res;
            //如果新值和已有值相等则直接返回
            if(($data['volume_closed']-$resData['volume_closed'])>0){
                $temp=$data;
                $temp['volume_closed']=$data['volume_closed']-$resData['volume_closed'];
                $resData['volume_closed']=$data['volume_closed'];
                $db->update('fxcmdata',$resData,"id='".$resData['id']."'");
            }else{
                return;
            }
        }else{
    ...
    private function tempDataConverter($temp){
        ...
        $rebateData['account']=$temp['account'];
        $rebateData['account_name']=$temp['customer_name'];
        $rebateData['fetype']=$temp['symbol'];
        $rebateData['quantity']=$temp['volume_closed'];
